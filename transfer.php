...
$sql = "INSERT INTO transactions (toCustomer,amount)
VALUES (".$id . "," .$amount .")";
...
function update_balance ( $id, $amount, $dbUpdate) {
    $sql = "SELECT currentBalance FROM customers WHERE id=$id";
    $result = $dbUpdate->query($sql);
    $oldAmount = $result->fetch_assoc();
    $oldAmount = $oldAmount['currentBalance'];
    $newAmount = $oldAmount + $amount;
    $sqlUpdate = "UPDATE customers SET currentBalance=$newAmount WHERE id=$id";
